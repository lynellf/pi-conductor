...
export function assertNoUnfinishedToolExecutions(records: readonly ToolExecutionRecord[]): void {
  ...
  if (
    timeline.unfinished.length > 0 ||
    timeline.entries.some((entry) => entry.finished?.outcome === "cleanup_unconfirmed")
  ) {
    throw new ToolExecutionError(
      "tool_resume_unknown_owner",
      "unfinished tool execution has unknown ownership; cleanup must be confirmed before resume",
      { cleanup: "unconfirmed" },
    );
  }
}
