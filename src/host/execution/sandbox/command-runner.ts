...
import {
  ...
  type SandboxOutputPreviews,
  type SandboxOutputSpool,
...
export function createSandboxCommandRunner(
  ...
  if (options.command.includes("\0")) throw new TypeError("sandbox command must be NUL-free");
  let stopped = false;
  ...
  async function doPrepare(value: ToolExecutionScope): Promise<SandboxReadyEvidence> {
    ...
    const args = buildSandboxMountPlan({
      ...
      immutableWorkspaceRoot: project.basePath,
    ...
    const startup = await Promise.race([pipes.startup, pipes.fault]);
    await Promise.race([pipes.ready, pipes.fault]);
    ...
    return Object.freeze({
      sandbox: { child_id: admission.childId, descriptor: admission.sandbox },
      boot_id: bootId,
  ...
  async function settle(): Promise<SandboxCommandResult> {
    ...
    try {
      normalizedStatus = await pipes.settlement;
    } catch (cause) {
