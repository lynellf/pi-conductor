...
### Feature
- **Bounded sub-agent delegation via the `delegate` host tool** (issue #17). An
  explicitly enabled role can call `delegate` with multiple independent tasks. The
  host runs those tasks concurrently up to `max_parallel`, waits for all task
  terminals, and returns an ordered structured result set to the parent role.
  The feature provides useful parallelism without creating a nested conductor.

### New host tool
- **`delegate`** — manifest-gated; opt-in. Accepts a batch of tasks with
  `id`, `objective`, `expected_output`, and `workspace` (`read_only` or
  `worktree`). Validates task count, workspace permissions, depth, and bounded
  strings before spawning any child.

### New child tool
- **`report_result`** — host-bound; required for child terminal reporting.
  Children must call this tool exactly once with `status`, `summary`, and
  optional `verification` lines.

### New worktree tool
- **`run`** — argv-based; restricted allowlist; path-confined. Replaces
  unrestricted `bash` for worktree children. Accepts a command name from an
  approved list (e.g., `git`, `pnpm`, `npm`, `node`) and an argv array with
  no shell interpolation.

### New records (host-agnostic, additive)
- **`subagent_started`** — written when the host starts a child auxiliary
  session.
- **`subagent_completed`** — written when a child succeeds or reports
  `no_changes`.
- **`subagent_failed`** — written when a child fails, is cancelled, or is
  recovered on resume.

### New manifest field
- **`roles[].delegation`** — opt-in block controlling delegation policy:
  `max_parallel`, `max_children`, `max_depth`, `workspace_modes`, and
  `max_child_cost_usd`. A role must have both a `delegation:` block and
  `delegate` in `tools` to use delegation.

### New cost view
- **`rollup.perSubagent`** — additive; children do not appear in `perRole`.
  Child usage appears exactly once in run cost roll-up and run-cap admission.

### Notes
- **Budget reservation:** delegated usage is charged to the parent invocation's
  session budget for admission/cap purposes. A `ChildBudgetLedger` tracks
  reserved `max_child_cost_usd` per child and settles on terminal with the
  actual cost. A delegated batch cannot breach the orchestrator's run cap.
- **Cancellation propagation:** `RunHandle.abort()` aborts all active children,
  each getting a `subagent_failed` with `status: "cancelled"`. Cancellation
  is idempotent.
- **Resume reconciliation:** `resumeRun` scans for orphan children
  (`subagent_started` with no terminal), reconciles them idempotently, and
  safely removes only clean conductor-owned worktrees. Dirty worktrees are
  preserved.
- No new package dependencies; no reducer change; no FSM contract change; no
  breaking change to existing public API.
- The `delegate` and `report_result` TypeBox schemas are the single source of
  truth for tool-arg shape.
- See `docs/decisions/ADR-002-subagent-delegation.md`.
