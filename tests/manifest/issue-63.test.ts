...
describe("Issue #63 manifest handoff policies", () => {
  ...
  it.each([
    ...
    ["unknown key", "handoffs: [{ from: planner, to: orchestrator, mode: fresh, extra: no }]", "unknown key"],
  ...
  it.each([
    ["undeclared endpoints", `[{ from: ghost, to: phantom, mode: fresh }]`, ["handoff-policy-from-undeclared", "handoff-policy-to-undeclared"]],
    ["duplicate directed edge", `[{ from: planner, to: orchestrator, mode: fresh }, { from: planner, to: orchestrator, mode: trajectory }]`, ["handoff-policy-duplicate-edge"]],
    ["self edge", `[{ from: planner, to: planner, mode: fresh }]`, ["handoff-policy-self-edge"]],
    ["worker to worker edge", `[{ from: planner, to: implementer, mode: fresh }]`, ["handoff-policy-illegal-edge"]],
    ["trajectory isolated workspace", `[{ from: planner, to: orchestrator, mode: trajectory }]`, ["trajectory-workspace-unsupported"]],
    ["trajectory delegation tool", `[{ from: planner, to: orchestrator, mode: trajectory }]`, ["trajectory-custom-tool-unsupported"]],
    ["trajectory target without model", `[{ from: planner, to: orchestrator, mode: trajectory }]`, ["trajectory-target-model-unresolved"]],
    ["trajectory target without prompt", `[{ from: planner, to: orchestrator, mode: trajectory }]`, ["trajectory-target-system-prompt-unresolved"]],
    ...
    if (_name === "trajectory isolated workspace") yaml = yaml.replace("name: planner\n    max_visits", "name: planner\n    workspace: { backend: worktree }\n    max_visits");
    if (_name === "trajectory delegation tool") yaml = yaml.replace("system_prompt: .pi/roles/planner.md", "system_prompt: .pi/roles/planner.md\n    tools: [delegate]\n    delegation: { allowed_subagents: [child], max_children_per_session: 1, max_parallel: 1 }");
    if (_name === "trajectory target without model") yaml = yaml.replace("    models: [{ model: stub:orchestrator, effort: medium }]\n", "");
    if (_name === "trajectory target without prompt") yaml = yaml.replace("    system_prompt: .pi/roles/orchestrator.md\n", "");
