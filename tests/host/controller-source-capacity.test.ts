...
describe("production source preparation capacity", () => {
  ...
  it("emits the exact required vs approved bytes in the storage diagnostic", async () => {
    const root = await mkdtemp(join(tmpdir(), "conduct-source-capacity-diag-"));
    roots.push(root);
    const repository = join(root, "repository");
    await mkdir(repository, { mode: 0o700 });
    const git = async (...args: string[]) =>
    ...
    const definition = approveControllerDefinition("source-capacity-diag", config, approval, 1);
    const activation: ControllerActivationStartedRecord = {
      type: "controller_activation_started",
      schema_version: 1,
      run_id: definition.record.run_id,
      controller_id: "planner",
      definition_digest: definition.record.definition_digest,
      activation_id: "capacity-diag-activation",
      owner_epoch: 1,
      previous_activation_id: null,
      reason: "start",
      ts: 2,
    ...
    const scope: ToolExecutionScope = {
      executionId: "capacity-diag-execution",
      supervisionId: "capacity-diag-supervision",
      signal: new AbortController().signal,
      graceMs: 1000,
      remainingTimeoutMs: () => 30_000,
      assertOpen: () => undefined,
    ...
    const perWorkspace = 8 * maxSourceBytes + maxSourceFiles * 512 * 1024;
    const reserved = 1;
    const required = (reserved + 1) * perWorkspace;
    const approved = maxWorkspaces * 16 * 1024 * 1024;
    const firstAction = action("first");
    const first = dispatcher.prepare(
      firstAction,
      controllerActionRequestDigest(definition.record.definition_digest, firstAction),
