 ...
 *     for (Pinned #4).
 *
 * Phase 1 (open-issues-round-2): `conduct.role.text_stream`
 * renderer removed — text now emits as a single `conduct.role.text`
 * per assistant turn, so the label-less continuation renderer
 * is no longer needed.
 *
...
describe("createConductMessageRenderers", () => {
  it("returns renderers for conduct.role.text and conduct.role.tool", () => {
  ...
  // Phase 1 (open-issues-round-2): `conduct.role.text_stream` renderer
  // removed — text now emits as a single `conduct.role.text` per assistant
  // turn, so the label-less continuation renderer is no longer needed.
});
