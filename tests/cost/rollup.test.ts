...
// ─── Issue #17: child terminal usage rollup ─────────────────────────────

function mkSubUsage(input: number, output: number, cost: number) {
  return mkUsage(input, output, 0, 0, cost);
}

function subCompleted(opts: {
  run_id?: string;
  child_id: string;
  task_id: string;
  model: string | null;
  usage: ReturnType<typeof mkUsage>;
  ts?: number;
}) {
  return {
    ...
    session_file: `/subagent-${opts.child_id}.jsonl`,
    attempt: 1,
    model: opts.model,
    ...
    summary: "done",
    usage: opts.usage,
    ts: opts.ts ?? 1_800_000_000_000,
  };
}

function subFailed(opts: {
  run_id?: string;
  child_id: string;
  task_id: string;
  model: string | null;
...
function subStarted(opts: { run_id?: string; child_id: string }) {
  return {
    type: "subagent_started" as const,
    run_id: opts.run_id ?? RUN_A,
    child_id: opts.child_id,
    task_id: `task-${opts.child_id}`,
    parent_role: "implementer",
    parent_session: "/parent.jsonl",
    session_file: `/subagent-${opts.child_id}.jsonl`,
    attempt: 1,
...
describe("rollup: includes child terminal usage (issue #17)", () => {
  it("orchestrator session plus child subagent_completed sum in perRun", () => {
    const records: PersistedRecord[] = [
      ended({
        role: "orchestrator",
        model: "anthropic:claude-sonnet-4-5",
      ...
      subCompleted({
        ...
        task_id: "task-1",
        model: "anthropic:claude-opus-4-5",
        usage: mkSubUsage(200, 100, 1.0),
      }),
    ];
    const result = rollup(records, RUN_A, "orchestrator");
    // 0.5 (orch) + 1.0 (child) = 1.5
    expect(result.perRun.cost).toBeCloseTo(1.5, 6);
    expect(result.perRun.sessions).toBe(2);
  });

  it("child model appears in perModel under the child's model string", () => {
    const records: PersistedRecord[] = [
      subCompleted({
        child_id: "child-a",
        task_id: "task-1",
    ...
    const result = rollup(records, RUN_A, "orchestrator");
    expect(result.perModel["anthropic:claude-opus-4-5"]?.cost).toBeCloseTo(1.0, 6);
    expect(result.perModel["anthropic:claude-opus-4-5"]?.sessions).toBe(1);
  });

  it("perSubagent has one row per child_id with that child's usage", () => {
    const records: PersistedRecord[] = [
      subCompleted({
        child_id: "child-a",
        task_id: "task-1",
      ...
      subFailed({
        ...
        task_id: "task-2",
        model: "openai:gpt-4o",
        usage: mkSubUsage(100, 50, 0.5),
        reason: "model_error",
      }),
    ];
    const result = rollup(records, RUN_A, "orchestrator");
    expect(Object.keys(result.perSubagent)).toHaveLength(2);
    expect(result.perSubagent["child-a"]?.cost).toBeCloseTo(1.0, 6);
    expect(result.perSubagent["child-b"]?.cost).toBeCloseTo(0.5, 6);
    expect(result.perSubagent["child-a"]?.sessions).toBe(1);
    expect(result.perSubagent["child-b"]?.sessions).toBe(1);
  });

  it("perRole is unchanged for a run with no main role sessions (children only)", () => {
    const records: PersistedRecord[] = [
      subCompleted({
        child_id: "child-a",
        task_id: "task-1",
    ...
    const result = rollup(records, RUN_A, "orchestrator");
    // Children do NOT appear in perRole (no double-attribution, spec §12.1)
    expect(result.perRole.orchestrator).toBeUndefined();
    expect(result.perRole.implementer).toBeUndefined();
  });

  it("perRole still reflects main sessions alongside children (no interference)", () => {
    const records: PersistedRecord[] = [
      ended({
        role: "implementer",
        model: "anthropic:claude-opus-4-5",
    ...
    const result = rollup(records, RUN_A, "orchestrator");
    // Main session still counted in perRole
    expect(result.perRole.implementer?.cost).toBeCloseTo(0.5, 6);
    expect(result.perRole.implementer?.sessions).toBe(1);
    // Child in perSubagent but NOT in perRole
    expect(result.perSubagent["child-a"]?.cost).toBeCloseTo(1.0, 6);
  });

  it("subagent_started has no usage and does not contribute to the rollup", () => {
    const records: PersistedRecord[] = [
      subStarted({ child_id: "child-a" }),
      subCompleted({
        child_id: "child-a",
        task_id: "task-1",
        model: "anthropic:claude-opus-4-5",
    ...
    const result = rollup(records, RUN_A, "orchestrator");
    // Only the terminal record contributes; sessions count = 1 (terminal only)
    expect(result.perRun.sessions).toBe(1);
    expect(result.perRun.cost).toBeCloseTo(1.0, 6);
  });

  it("run_id filtering applies to subagent records", () => {
    const records: PersistedRecord[] = [
      subCompleted({
        run_id: RUN_A,
        child_id: "child-a",
      ...
      subCompleted({
        ...
        task_id: "task-2",
        model: "anthropic:claude-opus-4-5",
        usage: mkSubUsage(300, 150, 2.0),
      }),
    ];
    const result = rollup(records, RUN_A, "orchestrator");
    expect(result.perRun.cost).toBeCloseTo(1.0, 6);
    expect(result.perRun.sessions).toBe(1);
    expect(Object.keys(result.perSubagent)).toHaveLength(1);
    expect(result.perSubagent["child-a"]).toBeDefined();
    expect(result.perSubagent["child-b"]).toBeUndefined();
  });

  it("children from other runs do not appear in perSubagent", () => {
    const records: PersistedRecord[] = [
      subCompleted({
        run_id: RUN_B,
        child_id: "child-b",
    ...
    const result = rollup(records, RUN_A, "orchestrator");
    expect(result.perRun.cost).toBe(0);
    expect(Object.keys(result.perSubagent)).toHaveLength(0);
  });

  it("perRun tokens include child tokens", () => {
    const records: PersistedRecord[] = [
      ended({
        role: "orchestrator",
        model: "anthropic:claude-sonnet-4-5",
    ...
    const result = rollup(records, RUN_A, "orchestrator");
    // mkUsage(100,50,0,0,0.5) tokens=150; mkSubUsage(200,100,1.0) tokens=300; total=450
    expect(result.perRun.tokens).toBe(450);
  });

  it("existing rollup test cases pass unchanged", () => {
    // Verify that the base rollup cases still hold: a single orchestrator
    // session with no children produces perRun, perRole, perModel, and
    // empty perSubagent.
    const records: PersistedRecord[] = [
      ended({
        role: "orchestrator",
    ...
    const result = rollup(records, RUN_A, "orchestrator");
    expect(result.perRun.cost).toBeCloseTo(0.5, 6);
    expect(result.perRole.orchestrator?.cost).toBeCloseTo(0.5, 6);
    expect(Object.keys(result.perSubagent)).toHaveLength(0);
  });
});
